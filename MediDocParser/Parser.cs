...
namespace MediDocParser
{
    public static class Parser
    {
        ...
        public static IEnumerable<ExecutingDoctor> ParseTextReport(TextReader reader)
        {
            ...
            {
                if (line != null)
                    executingDoctors.Add(ParseTextReportDoctorBlock(reader, ref lineNumber, line));
                lineNumber++;
            }
        ...
        public static IEnumerable<Lab> ParseLabReport(TextReader reader)
        {
            ...
            {
                if (line != null)
                    labs.Add(ParseLabBlock(reader, ref lineNumber, line));
                lineNumber++;
            }
            while ((line = reader.ReadLine()) != null);
        ...
        static Lab ParseLabBlock(TextReader reader, ref int lineNumber, string line)
        {
            ...
            lineNumber++;
            lab.Name = reader.ReadLine()?.CheckMaxSize(50, lab.ParserErrors, lineNumber).Trim();
            lineNumber++;
            lab.Address1 = reader.ReadLine()?.CheckMaxSize(50, lab.ParserErrors, lineNumber).Trim();
            lineNumber++;
            lab.Address2 = reader.ReadLine()?.CheckMaxSize(50, lab.ParserErrors, lineNumber).Trim();
            lineNumber++;
            lab.IdentificationData1 = reader.ReadLine()?.CheckMaxSize(50, lab.ParserErrors, lineNumber).Trim();
            lineNumber++;
            lab.IdentificationData2 = reader.ReadLine()?.CheckMaxSize(50, lab.ParserErrors, lineNumber).Trim();

            //Lijn 7: datum (+ eventueel tijdstip) aanmaak
            //formaat: JJJJMMDD(+evtHHMM)
            lineNumber++;
            lab.Date = reader.ReadLine()?.ToNullableDatetime("yyyyMMddHHmm", "yyyyMMdd", "yyMMdd");
            ...
            lineNumber++;
            var name = reader.ReadLine()?.CheckMaxSize(40, lab.ParserErrors, lineNumber);
            ...
            do
            {
                ...
                if (line != null)
                    lab.Patients.Add(ParsePatientBlock(reader, ref lineNumber, lab.ParserErrors, line, true));
        ...
        static Patient ParsePatientBlock(TextReader reader, ref int lineNumber, IDictionary<int, IList<string>> parserErrors, string firstLine, bool lab)
        {
            ...
            lineNumber++;
            var name = reader.ReadLine()?.CheckMaxSize(40, parserErrors, lineNumber);
            ...
            //formaat: JJJJMMDD
            lineNumber++;
            patient.BirthDate = reader.ReadLine()?.CheckMaxSize(8, parserErrors, lineNumber).ToNullableDatetime("yyyyMMdd", "yyMMdd");

            //Lijn 4: geslacht patiënt
            //formaat: X, of Y, of Z
            lineNumber++;
            switch (reader.ReadLine()?.CheckMaxSize(1, parserErrors, lineNumber))
            {
                case "X":
            ...
            //formaat: JJJJMMDD
            lineNumber++;
            patient.RequestDate = reader.ReadLine()?.CheckMaxSize(8, parserErrors, lineNumber).ToNullableDatetime("yyyyMMdd", "yyMMdd");

            //(Lijn 6:	referentienummer aanvraag 
            //formaat: max 14 karakters.
            lineNumber++;
            patient.ReferenceNumber = reader.ReadLine()?.CheckMaxSize(14, parserErrors, lineNumber).Trim();
            ...
            if (lab)
            {
                ...
                lineNumber++;
                var protocolCode = reader.ReadLine()?.CheckMaxSize(1, parserErrors, lineNumber);
                switch (protocolCode)
                {
                    case "P":
            ...
            else
            {
                ...
                lineNumber++;
                patient.EpisodeNumber = reader.ReadLine()?.CheckMaxSize(14, parserErrors, lineNumber).Trim();
            }
            ...
            do
            {
                ...
                if (line != null)
                    patient.Results.Add(ParseResultBlock(reader, ref lineNumber, parserErrors, line));
                lineNumber++;
            }
        ...
        static Result ParseResultBlock(TextReader reader, ref int lineNumber, IDictionary<int, IList<string>> parserErrors, string firstLine)
        {
            ...
            if (firstLine == "#Rb")
                return ParseTextResultBlock(reader, ref lineNumber, parserErrors);
            if (firstLine == "#Rc")
                return ParseResultTitleBlock(reader, ref lineNumber, parserErrors);
        ...
        static NumericResult ParseNumericBlock(TextReader reader, ref int lineNumber, IDictionary<int, IList<string>> parserErrors, string firstLine)
        {
            ...
            lineNumber++;
            result.Value = reader.ReadLine()?.Trim();
            ...
            //formaat: 2 karakters
            lineNumber++;
            result.Unit = reader.ReadLine()?.CheckMaxSize(2, parserErrors, lineNumber).Trim();

            //Lijn 5:	aanduiding pathologisch/normaal (max. 6 karakters)
            lineNumber++;
            var intensity = reader.ReadLine()?.CheckMaxSize(6, parserErrors, lineNumber);
        ...
        static ExecutingDoctor ParseTextReportDoctorBlock(TextReader reader, ref int lineNumber, string line)
        {
            ...
            lineNumber++;
            var name = reader.ReadLine()?.CheckMaxSize(40, executingDoctor.ParserErrors, lineNumber);
            ...
            //uitvoerend arts of paramedicus
            lineNumber++;
            var address = reader.ReadLine()?.CheckMaxSize(45, executingDoctor.ParserErrors, lineNumber);
            ...
            //uitvoerend arts of paramedicus
            lineNumber++;
            address = reader.ReadLine()?.CheckMaxSize(45, executingDoctor.ParserErrors, lineNumber);
            ...
            //uitvoerend arts of paramedicus
            lineNumber++;
            executingDoctor.Phone = reader.ReadLine()?.CheckMaxSize(50, executingDoctor.ParserErrors, lineNumber).Trim();

            //(lijn 6:)Boodschap (vrije tekst) (positie 1-50)
            lineNumber++;
            executingDoctor.Message = reader.ReadLine()?.CheckMaxSize(50, executingDoctor.ParserErrors, lineNumber).Trim();

            //(lijn 7:)Datum(+eventueel tijdstip) aanmaak diskette (positie 1-10)
            //formaat: JJJJMMDD(+evtHHMM)
            executingDoctor.Date = reader.ReadLine()?.ToNullableDatetime("yyyyMMddHHmm", "yyyyMMdd", "yyMMdd");
            ...
            //formaat: C/CCCCC/CC/CCC
            lineNumber++;
            executingDoctor.RequestingDoctor.RizivNr = reader.ReadLine()?.Replace("/", string.Empty).Trim();

            //(lijn 9:)Naam (positie 1-24) + Voornaam (positie 25-40)
            //aanvragende arts
            lineNumber++;
            name = reader.ReadLine()?.CheckMaxSize(40, executingDoctor.ParserErrors, lineNumber);
            ...
            do
            {
                ...
                if (line != null)
                    executingDoctor.Patients.Add(ParsePatientBlock(reader, ref lineNumber, executingDoctor.ParserErrors, line, false));
                lineNumber++;
            }
            while ((line = reader.ReadLine()) != null && !line.StartsWith(@"#/"));
